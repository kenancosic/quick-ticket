...
//encript and sign token
export async function signAuthToken<T>(payload: T) {
  try {
...
export async function setAuthCookie(token: string) {
  try {
    ...
    logEvent('Cookie setting failed', 'auth', {tokenSnippet: token.slice(0, 10)}, 'error', error);
  }
}
...
export async function removeAuthCookie() {
  try {
    ...
    logEvent('Cookie removing failed', 'auth', {}, 'error', error);
  }
}